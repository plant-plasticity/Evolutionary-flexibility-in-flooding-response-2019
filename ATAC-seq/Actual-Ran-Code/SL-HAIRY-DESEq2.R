...
directory <- "/Volumes/Deal-5TB/SUBEXP-ATAC/Tomato2/CountResults/HAIRY"
sampleFiles <- grep("counts",list.files(directory),value=TRUE)
sampleFiles
SampleDay <- c("DayOne", "DayOne", "DayTwo", "DayTwo", "DayTwoB", "DayTwoB")
sampleCondition <- c("CON", "SUB", "SUB", "CON", "CON", "SUB")
Names <- c("SL-HAIRY-CON-1", "SL-HAIRY-SUB-1", "SL-HAIRY-SUB-2", "SL-HAIRY-CON-2", "SL-HAIRY-CON-2B", "SL-HAIRY-SUB2-2")
...
ddsNORM <- dds
sizeFactors(ddsNORM) <- c(1.2236507021182, 0.9109338696030, 1.2078419973548, 0.7546604222355, 1.0420925765999, 0.8608204320887)
sizeFactors(ddsNORM)      
...
dev.copy(png, "SL-HAIRY-MAplot-ddsNorm.png")
dev.off()
...
sum(resNORM$padj < 0.1 & resNORM$log2FoldChange > 1, na.rm=TRUE )
sum(resNORM$padj < 0.1 & resNORM$log2FoldChange < -1, na.rm=TRUE )
sum(resNORM$pvalue < 0.1 & resNORM$log2FoldChange > 1, na.rm=TRUE )
sum(resNORM$pvalue < 0.1 & resNORM$log2FoldChange < -1, na.rm=TRUE )

sum(resNORM$padj < 0.1 & resNORM$log2FoldChange > 0.5, na.rm=TRUE )
sum(resNORM$padj < 0.1 & resNORM$log2FoldChange < -0.5, na.rm=TRUE )
sum(resNORM$pvalue < 0.1 & resNORM$log2FoldChange > 0.5, na.rm=TRUE )
sum(resNORM$pvalue < 0.1 & resNORM$log2FoldChange < -0.5, na.rm=TRUE )

sum(resNORM$padj < 0.05 & resNORM$log2FoldChange > 0.5, na.rm=TRUE )
sum(resNORM$padj < 0.05 & resNORM$log2FoldChange < -0.5, na.rm=TRUE )
sum(resNORM$pvalue < 0.05 & resNORM$log2FoldChange > 0.5, na.rm=TRUE )
sum(resNORM$pvalue < 0.05 & resNORM$log2FoldChange < -0.5, na.rm=TRUE )


rld <- rlogTransformation(ddsNORM, blind=T)
print(plotPCA(rld, intgroup = c("condition","Batch")))
dev.copy(png, "SL-HAIRY-PCA-rld-Norm.png")
dev.off()
...
head(resNORMdFrame)
write.csv(resNORMdFrame, file="SL-HAIRY-ATAC-NormalizedResults.csv")
...
with(resNORMdFrame, plot(log2FoldChange, -log10(padj), pch=20, main="Volcano plot", ylim=c(0,20)))
...
dev.copy(png, "SL-HAIRY-VOLCANO-FORM.png")
dev.off()
