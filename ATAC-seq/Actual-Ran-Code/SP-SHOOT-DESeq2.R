...
directory <- "/Volumes/Deal-5TB/SUBEXP-ATAC/Tomato2/CountResults/SP-SHOOT"
sampleFiles <- grep("counts",list.files(directory),value=TRUE)
sampleFiles
SampleDay <- c("DayTwo", "DayTwo", "DayOne", "DayOne")
sampleCondition <- c("CON", "SUB","CON", "SUB")
Names <- c("SP-SHOOT-CON-2", "SP-SHOOT-SUB-2", "SP-SHOOT-CON-1", "SP-SHOOT-SUB-1")
...
ddsNORM <- dds
sizeFactors(ddsNORM) <- c(1.451579814814780, 0.896709855715832, 0.651710329469390, 1.333065960935410)
sizeFactors(ddsNORM)      
...
dev.copy(png, "SP-SHOOT-MAplot-ddsNorm.png")
dev.off()
...
dev.copy(png, "SP-SHOOT-PCA-rld-Norm.png")
dev.off()
...
head(resNORMdFrame)
write.csv(resNORMdFrame, file="SP-SHOOT-resTestTwo-NormalizedResults.csv")
